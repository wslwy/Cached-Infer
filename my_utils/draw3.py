...
if __name__ == "__main__":
    ...
    if model_type == "vgg16_bn":
        ...
        file = "results/resnet101_samll_valid_test2.pkl"

    with open(file, 'rb') as fi:
        loaded_data = pickle.load(fi)

    cache_sign_list     = loaded_data["cache_sign_list"]
    ...
    if model_type == "resnet101":
        select_index = [0, 3, 7, 11, 12, 13]
        # select_index = [0, 3, 11, 13]

    draw_cache_sign_list = []
    draw_avg_time_list = []
    draw_correct_ratio_list = []

    for idx in select_index:
        draw_cache_sign_list.append(cache_sign_list[idx])
        draw_avg_time_list.append(avg_time_list[idx])
        draw_correct_ratio_list.append(correct_ratio_list[idx])
    ...
    draw_correct_ratio_list[0] = draw_correct_ratio_list[2]
    ...
    # 设置柱状图宽度和横坐标位置
    bar_width = 0.3  # 柱状图宽度
    x_axis_positions = np.arange(len(x))  # 横坐标位置
    ...
    plt.xticks(x_axis_positions, x)  # 设置x轴刻度的位置

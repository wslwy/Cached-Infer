...
if __name__ == "__main__":
    ...
    # 测试对比两个
    sign_id_lists = [ sign_id_lists[0], sign_id_lists[8], sign_id_lists[11] ]
    ...
    for idx, sign_list in enumerate(sign_lists):
        ...
        print(f"idx: {idx}, sign list : {sign_list} test start ...")
        for layer in range(global_cache.cache_layer_num):
            dim = len(global_cache.cache_table[layer][0])
            local_cache.cache_table[layer] = np.zeros((cache_size, dim), dtype=float)

        avg_time, correct_ratio = cached_infer(sub_models, model_type, global_cache, local_cache, test_loader, device, cache_size)
    ...
    if model_type == "vgg16_bn":
        ...
        file = "results/cache_50_cpmpare_test.pkl"
